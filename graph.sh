...
import re, os, sys, pwd, optparse, errno, tarfile, subprocess
...
def showtable(rows, fields, columns, sort):
    ...
    if options.pie:
        var = showpie(l, sort)[0]
        var1 = showpie(l, sort)[1]
        subprocess.Popen("echo %s > /fich_value"%var, shell=True)
        subprocess.Popen("echo %s > /fich_labels"%var1, shell=True)
        return
